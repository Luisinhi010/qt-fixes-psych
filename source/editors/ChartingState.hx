...
class ChartingState extends MusicBeatState
{
	public static var noteTypeList:Array<String> = // Used for backwards compatibility with 0.1 - 0.3.2 charts, though, you should add your hardcoded custom note types here too.
		[
			'', 'Alt Animation', 'Hey!', 'Hurt Note', 'GF Sing', 'No Animation', 'Invisible Hurt Note', 'Kb Note', 'Kb Note No Animation', 'Miss Note',
			'Invert Anim Note'
		];
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'set vignette',
			"Value 1 refers the time \n\nValue 2 is for the alpha to fade to or for pre mades one.\n'bump': will do like Alarm Gradient but with vignette.\n'camerabump':same thing but with without starting apaear \nOnly works if Low Quality is disabled."
		],
		['set camzoom', "Value 1: time \nValue 2: zoom."],
		['reset notes pos', "Reset the notes positions \nValue 1: time "],
		[
			'notes modchart opponent',
			"notes modchart but to opponent notes \nValue 1: X \nValue 2: Y "
		],
		[
			'notes modchart player',
			"notes modchart but to player notes \nValue 1: X \nValue 2: Y "
		],
		[
			'censory Chroma',
			"censory overload custom Chroma shader \nValue 1: Intensity of the effect \nValue 2: the Limit of the effect"
		],
		[
			'TerminationIntro',
		...
		[
			'censory overload events',
			"Mid song events ported from original qt-fixes. \nit works on all songs, you can try\n something with this."
		],
		[
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadAutosaveBtn:FlxButton = new FlxButton(reloadSongJson.x, reloadSongJson.y + 30, 'Load Autosave', function()
		{
			...
			else if (FlxG.save.data.autosave != null)
			{
				...
				openSubState(new Prompt('You dont have any auto save!', 0, null, null, false, 'OK', 'disabledbutton'));
		});
		...
		// dodgeEnabled
		var check_dodging = new FlxUICheckBox(difficultyDropDown.x, player2DropDown.y, null, null, "Allow dodging", 100);
		...
		var check_haveoldvoices = new FlxUICheckBox(check_dodging.x, check_dodging.y + 40, null, null, "haveoldvoices", 100);
		check_haveoldvoices.checked = _song.dodgeEnabled;
		check_haveoldvoices.callback = function()
		{
			_song.haveoldvoices = check_haveoldvoices.checked;
		};

		var checkAutoZoom = new FlxUICheckBox(check_dodging.x, check_haveoldvoices.y + 40, null, null, "Auto-Zoom on Beat", 100);
		...
		tab_group_song.add(check_haveoldvoices);
	...
	function addChartingUI()
	{
		...
		sliderRate.setTexts("Playback speed", true, "0.5", "3.0", 10);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput && !fuckingCheater)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				fuckingtext();
				autosaveSong();
				LoadingState.loadAndSwitchState(new editors.EditorPlayState(sectionStartTime()), false, false);
			}
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				PlayState.chartingMode = true; // Pr: https://github.com/ShadowMario/FNF-PsychEngine/pull/10557
			}
		...
		curRenderedNotes.forEachAlive(function(note:Note)
		{
			...
			if (note.strumTime <= Conductor.songPosition)
			{
				...
				if (note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1)
				{
					...
					} // Updated so dosent play the strum animation on (hitCausesMiss) notes -Luis
					if (!playedSound[data])
					{
	...
	function loadJson(song:String):Void
	{
		if (song.toLowerCase() == "interlope")
		{
			...
			// the fact that was commented from the original code on playstate make me laugh xd
			...
			new FlxTimer().start(1.5, function(tmr:FlxTimer)
			{
				...
				FlxG.camera.fade(FlxColor.BLACK, 0.1, false, FlxG.resetGame, false);
			});
		}
		else
		{
	...
	{
		var txt:FlxText;
		txt = new FlxText(0, 0, 0, 'Loading...', 64,
