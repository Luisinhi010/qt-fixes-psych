...
// well, you can see in the name that this is broken. -Luis
...
class FreeplayState extends MusicBeatState
{
	...
	function onMouseDown(object:FlxObject)
	{
		for (idx in 0...grpSongs.members.length)
		{
			...
			if (obj == object || icon == object)
			{
				if (idx != curSelected)
				{
					changeSelection(idx, true, false);
				}
				else
				{
					selectSong();
		...
		/*if (!ClientPrefs.lowQuality)
			{
				...
				qt_gas.frames = Paths.getSparrowAtlas('hazard/qt-port/stage/Gas_Release', 'shared');
	...
	public static var instPlayingtxt:String = "N/A"; // its not really a text but who cares?
	...
	function changeSelection(change:Int = 0, playSound:Bool = true, additive:Bool = true)
	{
		if (playSound)
